...
## An Introduction To Limits {#sec:limit-intro}
...
```{geogebra, sinxoverx}
material_id: pbwtkz3r 
caption: "Graph of $f(x)=\\sin x/x$."
```
...
```{example, name="Rational function", label="limit1ex", echo=TRUE}
...
```{geogebra limit1}
material_id: ahxgeetr
caption: Graphically approximating the limit of a rational function.
```
...
\@ref(exm:limit1ex), we used both values less than and greater than 3. Had we
...
```{geogebra nolimit1}
material_id: qmysttuf
caption: "This function has no limit as $x \\to 1$."
```
...
```{example, echo=TRUE, name="Growth Without Bound", label="no-limit2"}
A graph and table of $f(x) = 1/(x-1)^2$ are given in Figure \@ref(fig:nolimit2) and Table \@ref(tab:nolimit2table), respectively. Both show that as
...
```{example, echo=TRUE, name="Oscillation"}
Explore why $\displaystyle \lim_{x\to 0}\sin(1/x)$ does not exist.

----
  
A graph of $f(x) = \sin(1/x)$ is given in Figure \@ref(fig:nolimit3).
...
```{geogebra nolimit3}
material_id: ewgs887d
caption: "A function that oscillates as $x\\to 0$"
```
...
```{geogebra diffquot}
material_id: cgazabpr
caption: "The secant lines of $f(x)$ at a point $a$. Move the slider for $h$ to investigate the limit of the difference quotient."
```
...
```{example, echo=TRUE, name="Square root function"}
...
```{example, echo=TRUE, name="Square function"}
...
```{example, echo=TRUE,name="Polynomial"}
...
```{example, echo=TRUE, name="Exponential"}
...
## Finding Limits Analytically {#sec:limit-analytically}

In Section \@ref(sec:limit-intro) we explored the concept of the limit
...
```{example, echo=TRUE, name="Using basic limit properties."}
...
```{example, echo=TRUE, name="Limit of a rational function", label="limrat"}
...
Example \@ref(exm:limrat).
```

```{example, echo=TRUE, name="Evaluating limits using algebra", label="limitonept"}
...
```{geogebra limitxplus1}
material_id: bbheqmd3
caption: "The function for Example \\@ref(exm:limitonept)."
```
...
```{example, echo=TRUE, name="Evaluating a limit using Theorem \\@ref(thm:limitallbut1)", label="limitallbut1ex"}
...
```{example, echo=TRUE, name="Evaluating limits analytically", label="limit2ex"}
...
1. This is a straightforward application of Theorem \@ref(thm:limcontinuous). $\displaystyle \lim_{x\to \pi} \cos x = \cos \pi = -1$.

1. We can approach this in at least two ways. First, by directly applying Theorem \@ref(thm:limcontinuous), we have:	$$\lim_{x\to 3} (\sec^2x - \tan^2 x) = \sec^23-\tan^23.$$ Using the Pythagorean Theorem, this last expression is 1; therefore $$\lim_{x\to 3} (\sec^2x - \tan^2 x) = 1.$$ We can also use the Pythagorean Theorem from the start. $$\lim_{x\to 3} (\sec^2x - \tan^2 x) = \lim_{x\to 3} 1 = 1,$$ using the Constant limit rule. Either way, we find the limit is 1.

1. Applying the Product limit rule of Theorem \@ref(thm:limitalgebra) and Theorem \@ref(thm:limcontinuous) gives $$\displaystyle \lim_{x\to \pi/2} \cos x\sin x = \cos (\pi/2)\sin(\pi/2) = 0\cdot 1 = 0.$$
...
We can also use the limit Composition Rule of Theorem \@ref(thm:limitalgebra). Using Theorem \@ref(thm:limcontinuous), we have $\displaystyle \lim_{x\to 1}\ln x = \ln 1 = 0$ and $\displaystyle\lim_{x\to 0} e^x= e^0=1$, satisfying the conditions of the Composition Rule. Applying this rule, $$\displaystyle \lim_{x\to 1} e^{\ln x} = \lim_{x\to 0} e^x = e^0 = 1.$$ Both approaches are valid, giving the same result.

1. We encountered this limit in Section \@ref(sec:limit-intro). Applying our theorems, we attempt to find the limit as $$\lim_{x\to 0}\frac{\sin x}{x}\rightarrow \frac{\sin 0}{0} \rightarrow \frac{0}{0}.$$ This, of course, violates a condition of Theorem \@ref(thm:limitalgebra), as the limit of the denominator is not allowed to be 0. Therefore, we are still unable to evaluate this limit with tools we currently have at hand.
```
...
```{example, label="limit-sinx-prove", echo=TRUE}
Use the Squeeze Theorem to show that $$\displaystyle \lim_{x\to 0} \frac{\sin x}{x} = 1.$$
...
Figure \@ref(fig:squeeze-sinx) shows three regions have been constructed in the first quadrant, two triangles and a sector of a circle, which are also drawn below. The area of the large triangle is $\frac12\tan\theta$; the area of the sector is $\theta/2$; the area of the triangle contained inside the sector is $\frac12\sin\theta$. It is then clear from the diagram that 

Multiply all terms by $\displaystyle\frac{2}{\sin \theta}$, giving $$\frac{1}{\cos\theta} \geq \frac{\theta}{\sin \theta} \geq 1.$$
...
Clearly this means that $\displaystyle \lim_{\theta\to 0} \frac{\sin\theta}{\theta}=1$.
```

```{geogebra, squeeze-sinx}
material_id: qwu4asbc
caption: >
  The unit circle and related triangles. The area of the triangle associated with $\sin \theta$
  is less than the area of the circular sector with angle $\theta$, which is less than the triangle
  associated with $\tan \theta$.
```
...
\@ref(sec:limit-intro), a limit of a difference quotient. Let
...
```{example, echo=TRUE, name="Evaluating the limit of a difference quotient", label="limitdiffquot"}
...
In Section \@ref(sec:limit-intro) we saw three ways in which limits of
...
One sided lmits are concerned with $x$ values on one side of the point $c$.
...
```{example, echo=TRUE, name="Evaluating one sided limits", label="onesidea"}
...
```{geogebra onesided1}
material_id: t9dpjyng
caption: "The function for Example \\@ref(exm:onesidea)"
```
...
One thing to consider in Examples \@ref(exm:onesidea) – \@ref(exm:onesided) is
...
```{example, echo=TRUE, name="Evaluating limits of a piecewise--defined function", label="onesideb"}
...
```{geogebra onesidedb}
material_id: y3zaqrhv
caption: "The function for Example \\@ref(exm:onesideb)"
```



```{example, echo=TRUE, name="Evaluating limits of a piecewise--defined function", label="onesidec"}
...
```{geogebra onesidedc}
material_id: rjr9ejfd
caption: "The function for Example \\@ref(exm:onesidec)"
```

```{example, echo=TRUE, name="Evaluating limits of a piecewise--defined function", label="onesided"}
...
```{geogebra onesidedd}
material_id: ryxpsrqc
caption: "The function for Example \\@ref(exm:onesided)"
```
  

In Examples \@ref(exm:onesidea) – \@ref(exm:onesided) we were asked to find
both $\displaystyle \lim_{x\to 1}f(x)$ and $f(1)$. Consider the following table:

  Example  | $\displaystyle \lim_{x\to 1}f(x)$ | $f(1)$
  :-----------------:  | :------------------:  | :------------:
  \@ref(exm:onesidea)   |    does not exist     |       1
  \@ref(exm:onesideb)   |          1            |   not defined
  \@ref(exm:onesidec)   |          0            |       1
  \@ref(exm:onesided)   |          1            |       1

Only in Example \@ref(exm:onesided) do both the function and the limit
