...
In Example \@ref(exm:implicit1) we found that
...
y &= x^x \\
\ln (y) &= $\ln (x^x)$ \text{(apply logarithm rule)}\\
\ln (y) &= $x\ln x$  \text{(now use implicit differentiation)}\\
\frac{d}{dx}\Big(\ln (y)\Big) &= \frac{d}{dx}\Big(x\ln x\Big) \\
\frac{y\,'}{y} &= \ln x + x\cdot\frac 1x\\
\frac{y\,'}{y} &= \ln x + 1\\
y\,' &= $y\big(\ln x+1\big)$ \text{(substitute $y=x^x$)}\\
