...
## Extreme Values {#sec:extreme_values}
...
::: {#fig:extreme .tab}
**Table 1**. caption: Graphs of functions with and without extreme
values.
:::
...
We follow the steps outlined in Key Idea \@ref(idea:extrema). We first
...
The table in Figure \@ref(table:ext4)(b) gives $f$ evaluated at the
...
::: {#table:ext4 .tab}
**Table 2**. caption: Finding the extreme values of
...
Figure \@ref(table:ext4) shows $f$ and we can confirm our answer, but it
...
graphed in Figure \@ref(table:ext5)(a).

------------------------------------------------------------------------

Here $f$ is piecewise--defined, but we can still apply Key Idea
\@ref(idea:extrema) as it is continuous on $[-4,2]$ (one should check to
...
Figure \@ref(table:ext5)(b). Thus the absolute minimum of $f$ is 1, the
absolute maximum of $f$ is $25$, confirmed by the graph of $f$.

::: {#table:ext5 .tab}
**Table 3**. caption: Finding the extreme values of a piecewise--defined
...
$[-2,2]$, graphed in Figure \@ref(table:ext6)(a).

------------------------------------------------------------------------

We again use Key Idea \@ref(idea:extrema). Evaluating $f$ at the
...
\@ref(table:ext6)(b). In this example we have 5 values to consider:
$x= 0, \pm 2, \pm\sqrt{\pi}$.

::: {#table:ext6 .tab}
**Table 4**. caption: Finding the extrema of $f(x)= \cos (x^2)$ in
Example \@ref(exm:extval6).
:::

From the table it is clear that the maximum value of $f$ on $[-2,2]$ is
...
```{example, label="extval7", echo=TRUE}

(Finding extreme values). Find the extreme values of
$f(x) = \sqrt{1-x^2}$, graphed in Figure \@ref(table:ext7)(a).
...
::: {#table:ext7 .tab}
**Table 5**. caption: Finding the extrema of the half--circle in Example
\@ref(exm:extval7).
:::
...
\@ref(table:ext7)(b). The maximum value is 1, and the minimum value is
...
> circle, in Example \@ref(exm:implicit7) as $\frac{dy}{dx} = -x/y$. In
> Example \@ref(exm:extval7), half of the unit circle is given as
