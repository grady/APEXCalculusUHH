...
$$\frac{f(b)-f(a)}{b-a} \Rightarrow \frac{\text{numerator }>0}{\text{denominator } >0} \Rightarrow \text{slope of the secant line $>0$} \Rightarrow \text{Average rate of change of $f$ on $[a,b]$ is $>0$.}$$
...
Using Key Idea \@ref(idea:incr-decr), we first find the critical values
...
3.  If f\^ is positive (or, negative) before and after $c$, then $f(c)$
...
Idea \@ref(idea:incr-decr) describes how to find intervals where $f$ is
...
apply the techniques of Key Idea \@ref(idea:incr-decr) to both intervals
of the domain of $f$.
...
Pick a number $p$ from each subinterval and test the sign of f\^ at $p$
...
Since at $x=-1$, the sign of f\^ switched from positive to negative,
Theorem \@ref(thm:first-der) states that $f(-1)$ is a relative maximum
of $f$. At $x=3$, the sign of f\^ switched from negative to positive,
...
$x=\pm 1$ and f\^ is not defined when $x=0$. Thus we have 3 critical
...
$(0,1)$. The sign of f\^ changes from negative to positive around $x=-1$
and $x=1$, meaning by Theorem \@ref(thm:first-der) that $f(-1)$ and
$f(1)$ are relative minima of $f$. As the sign of f\^ changes from
