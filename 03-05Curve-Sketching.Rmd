...
behavior we care about is explained by f\^ and f\^. Understanding the
interactions between the graph of $f$ and f\^ and f\^ is important. To
...
The following Key Idea summarizes what we have learned so far that is
...
(Curve sketching). Use Key Idea \@ref(idea:sketch) to sketch
$f(x) = 3x^3-10x^2+7x+5$.
...
2.  To find the critical values of $f$, we first find $f^{\prime}(x)$.
    ...
    when $x=-2,3$. Since f\^ is undefined only when $f$ is, these are
...
3.  To find the possible points of inflection, we find
    ...
    quadratic are imaginary) and f\^ is undefined when $x=-2,3$. Thus
