...
```{js}
var params = {"appName": "graphing", width: "auto", "showToolBar": true, "showMenuBar": true };
...
Table Caption: Approximating the instantaneous velocity with average
velocities over a small time period $h$.

[\[table:falling\]]{#table:falling label="table:falling"}
...
the point $(2,86)$. In (c) we see the secant line, which approximates
$f$ well, but not as well the tangent line shown in (d).\
...
denoted $f\,'(c)$, is $$\lim_{h\to 0}\frac{f(c+h)-f(c)}{h},$$ provided
...
1.  *We compute this directly using Definition
    \@ref(def:derivative-at-a-point). $$\begin{aligned}
...
*Figure Caption: A graph of $f(x) = 3x^2+5x-7$ and its tangent lines at
$x=1$ and $x=3$.*

*[\[fig:tangent1\]]{#fig:tangent1 label="fig:tangent1"}*
...
$$n(x) =\frac{-1}{f\,'(c)}(x-c)+f(c),$$ where $f\,'(c)\neq 0$. When
$f\,'(c)=0$, the normal line is the vertical line through
$\big(c,f(c)\big)$; that is, $x=c$.*

```
```{example, label="normal1", echo=TRUE}

(Finding equations of normal lines). *Let $f(x) = 3x^2+5x-7$, as in
Example \@ref(ex:derv-point1). Find the equations of the normal lines to
the graph of $f$ at $x=1$ and $x=3$.*

------------------------------------------------------------------------

*In Example \@ref(ex:derv-point1), we found that $f\,'(1)=11$. Hence at
...
*Figure Caption: A graph of $f(x)=3x^2+5x-7$, along with its normal line
at $x=1$.*

*[\[fig:normal1\]]{#fig:normal1 label="fig:normal1"}*
...
Figure Caption: $f(x) = \sin x$ graphed with an approximation to its
tangent line at $x=0$.

[\[fig:tangentsinx\]]{#fig:tangentsinx label="fig:tangentsinx"}

figures/figtangentsinx
...
this later.\
Consider again Example \@ref(ex:derv-point1). To find the derivative of
...
$$f\,'(x) = \lim_{h\to 0} \frac{f(x+h)-f(x)}{h}$$ is **the derivative of
$f$**.\
**Notation:***
...
Example \@ref(ex:derv-point1). Find $f\,'(x)$.*
...
$\displaystyle f(x) = \frac{1}{x+1}$. Find $f\,'(x)$.*
...
\@ref(ex:der-num-approx). $$\begin{aligned}
...
*Thinking back to Example \@ref(ex:der-num-approx), we can find the
...
*Figure Caption: The absolute value function, $f(x) = |x|$. Notice how
the slope of the lines (and hence the tangent lines) abruptly changes at
$x=0$.*

*[\[fig:absolutevalue\]]{#fig:absolutevalue label="fig:absolutevalue"}*

*figures/figabsolutevalue*
...
$$f\,'(0) = \lim_{h\to 0}\frac{f(0+h)-f(0)}{h}.$$ Since $x=0$ is the
...
*Figure Caption: A graph of the derivative of $f(x) = |x|$.*

*[\[fig:absolutevalueprime\]]{#fig:absolutevalueprime
label="fig:absolutevalueprime"}*

*figures/figabsolutevalueprime*
...
*Using Example \@ref(ex:deriv-sinx), we know that when $x<\pi/2$,
...
*Figure Caption: A graph of $f(x)$ as defined in Example
\@ref(ex:diff-piecewise).*

*[\[fig:piecewisesinx1\]]{#fig:piecewisesinx1
label="fig:piecewisesinx1"}*

*figures/figpiecewisesinx1*
...
*Figure Caption: A graph of $f\,'(x)$ in Example
\@ref(ex:diff-piecewise).*

*[\[fig:piecewisecosx1\]]{#fig:piecewisecosx1
label="fig:piecewisecosx1"}*

*figures/figpiecewisecosx1*
...
$f$ in Example \@ref(ex:diff-piecewise) is piecewise--defined, the
...
Example \@ref(ex:deriv1) we found that when $f(x) =  3x^2+5x-7$,
...
```{example, label="diff-closed1", echo=TRUE}
...
that $\lim_{x\to 0^+}f\,'(x) = \infty$; this limit was easier to
...
*Figure Caption: A graph of $y=x^{1/2}$ and $y=x^{3/2}$ in Example
\@ref(ex:diff-closed1).*

*[\[fig:diff-closed1\]]{#fig:diff-closed1 label="fig:diff-closed1"}*

*figures/figdiff-closed1*

------------------------------------------------------------------------

*We state (without proof) that $g\,'(x) = 3\sqrt{x}/2$. Note that
$\lim_{x\to 0^+}g\,'(x) = 0$; again, this limit is easier to evaluate
