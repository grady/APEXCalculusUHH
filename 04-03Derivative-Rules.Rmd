...
$y\,'$? Without limits, recognize that linear functions are
characterized by being functions with a constant rate of change (the
slope). The derivative, $y\,'$, gives the instantaneous rate of change;
with a linear function, this is constant, $m$. Thus $y\,'=m$.
...
$$f\,'(x) = \frac{d}{dx}\big(x\big) = \frac{d}{dx}\big(x^1\big) = 1\cdot x^0 = 1.$$
...
4.  *See Figure \@ref(fig:xcubedwithderiv).*
    ...
    *Figure Caption: A graph of $f(x) = x^3$, along with its derivative
    $f\,'(x) = 3x^2$ and its tangent line at $x=-1$.*

    *[\[fig:xcubedwithderiv\]]{#fig:xcubedwithderiv
    label="fig:xcubedwithderiv"}*

    *figures/figxcubedwithderiv*
...
1.  ***Sum/Difference Rule:***
    ...
    *$\displaystyle\frac{d}{dx}\Big(f(x) \pm g(x)\Big) = \frac{d}{dx}\Big(f(x)\Big) \pm \frac{d}{dx}\Big(g(x)\Big)$
    $= \rule{0pt}{15pt}f\,'(x)\pm g\,'(x)$*

2.  ***Constant Multiple Rule:***

    *$\displaystyle\frac{d}{dx}\Big(c\cdot f(x)\Big) = c\cdot\frac{d}{dx}\Big(f(x)\Big) = c\cdot f\,'(x)$.*
...
\@ref(ex:deriv1) that we found, using the limit definition, the
...
*Using Theorem \@ref(thm:deriv-common) we find $f\,'(x) = \cos x + 2$.
The slope of the tangent line is thus $f\,'(\pi) = \cos \pi + 2 =1$.
...
$f\,^{(4)}(x)$ notation, not $f\,''''(x)$; after a while, too many ticks
is confusing.\
...
1.  *Using the Power and Constant Multiple Rules, we have:
    $f\,'(x) = 8x$. Continuing on, we have
    $$f\,''(x) = \frac{d}{dx}\big(8x\big) = 8;\qquad f\,'''(x) = 0;\qquad f\,^{(4)}(x) = 0.$$
    Notice how all successive derivatives will also be 0.*

2.  *We employ Theorem \@ref(thm:deriv-common) repeatedly.
    $$f\,'(x) = \cos x;\qquad f\,''(x) = -\sin x;\qquad f\,'''(x) = -\cos x;\qquad f\,^{(4)}(x) = \sin x.$$
...
function. Then, as stated in Key Idea \@ref(idea:motion), $f\,'$
describes the rate of position change: velocity. We now consider
$f\,''$, which describes the rate of velocity change. Sports car
...
$f\,'(t)=-32t$ ft/s and $f\,''(t) = -32$ (ft/s)/s. We may recognize this
