...
\@ref(sec:limitintro) when we introduced the difference quotient. We have
...
of $h$ as seen in Figure \@ref(table:falling). It looks as though the
velocity is approaching $-64$ ft/s. Computing the limit directly gives
$$\begin{aligned}
\lim_{h\to 0} \frac{f(2+h)-f(2)}{h} &= \lim_{h\to 0}\frac{-16(2+h)^2+150 - (-16(2)^2+150)}{h} \\
...
Figure \@ref(fig:derivfalling)(a) shows a “zoomed out” version of $f$ with
its secant line. In (b), we zoom in around the points of intersection
between $f$ and the secant line. Notice how well this secant line
approximates $f$ between those two points – it is a common practice to
approximate functions with straight lines.
...
parts (c) and (d) of Figure \@ref(fig:derivfalling), we zoom in around the
point $(2,86)$. In (c) we see the secant line, which approximates $f$
well, but not as well the tangent line shown in (d).\
...
```{definition, label="derivativeatapoint", name="Derivative at a Point", echo=TRUE}

Let $f$ be a continuous function on an open interval $I$ and let $c$ be
in $I$. The [derivative of $f$ at $c$]{}, denoted ${f'}(c)$, is
$$\lim_{h\to 0}\frac{f(c+h)-f(c)}{h},$$ provided the limit exists. If
the limit exists, we say that [$f$ is differentiable at $c$]{}; if the
limit does not exist, then [$f$ is not differentiable at $c$]{}. If $f$
is differentiable at every point in $I$, then [$f$ is differentiable on
$I$]{}.

```

```{definition, label="tangentline", name="Tangent Line", echo=TRUE}

Let $f$ be continuous on an open interval $I$ and differentiable at $c$,
for some $c$ in $I$. The line with equation
$\ell(x) = {f'}(c)(x-c)+f(c)$ is the **tangent line** to the graph of
...
```{example, label="dervpoint1", name="Finding derivatives and tangent lines", echo=TRUE}
...
1.  We compute this directly using Definition
    \@ref(def:derivativeatapoint). $$\begin{aligned}
...
2.  The tangent line at $x=1$ has slope ${f'}(1)$ and goes through the
...
```{definition, label="normalline", name="Normal Line", echo=TRUE}

Let $f$ be continuous on an open interval $I$ and differentiable at $c$,
for some $c$ in $I$. The **normal line** to the graph of $f$ at $c$ is
the line with equation $$n(x) =\frac{-1}{{f'}(c)}(x-c)+f(c),$$ where
${f'}(c)\neq 0$. When ${f'}(c)=0$, the normal line is the vertical line
through $\big(c,f(c)\big)$; that is, $x=c$.

```

```{example, label="normal1", name="Finding equations of normal lines", echo=TRUE}

Let $f(x) = 3x^2+5x-7$, as in Example \@ref(exdervpoint1). Find the
equations of the normal lines to the graph of $f$ at $x=1$ and $x=3$.

------------------------------------------------------------------------

In Example \@ref(exdervpoint1), we found that ${f'}(1)=11$. Hence at
$x=1$, the normal line will have slope $-1/11$. An equation for the
normal line is $$n(x) = \frac{-1}{11}(x-1)+1.$$ The normal line is
plotted with $y=f(x)$ in Figure \@ref(fig:normal1). Note how the line looks
perpendicular to $f$. (A key word here is “looks.” Mathematically, we
say that the normal line *is* perpendicular to $f$ at $x=1$ as the slope
of the normal line is the opposite–reciprocal of the slope of the
...
We also found that ${f'}(3) = 23$, so the normal line to the graph of
...
functions with not–so–difficult functions. Lines are a common choice. It
turns out that at any given point on the graph of a differentiable
...
```{example, label="derline", name="Finding the derivative of a linear function", echo=TRUE}
...
is just $f$, with the same being true for $x=7$.
...
```{example, label="dernumapprox", name="Numerical approximation of the tangent line", echo=TRUE}

Approximate the equation of the tangent line to the graph of
$f(x)=\sin x$ at $x=0$.

------------------------------------------------------------------------

In order to find the equation of the tangent line, we need a slope and a
point. The point is given to us: $(0,\sin 0) = (0,0)$. To compute the
slope, we need the derivative. This is where we will make an
approximation. Recall that
$${f'}(0) \approx \frac{\sin(0+h)- \sin 0}{h}$$ for a small value of
$h$. We choose (somewhat arbitrarily) to let $h=0.1$. Thus
$${f'}(0) \approx \frac{\sin(0.1)-\sin 0}{0.1} \approx 0.9983.$$ Thus
...
\
Recall from Section \@ref(sec:limitanalytically) that
$ \lim_{x\to 0}\frac{\sin x}x =1$, meaning for values of $x$ near 0,
...
this later.\
Consider again Example \@ref(exdervpoint1). To find the derivative of
...
(0,0) node (n1) \[text width=60pt,align=center\] [input specific number
$c$ ]{} (3,0) node (n2) \[draw,text width=60pt,align=center\] [do
something to $f$ and $c$]{} (6,0) node (n3) \[text
width=60pt,align=center\] [return number ${f'}(c)$]{}; (n1) – (n2); (n2)
– (n3);

This process describes a *function*; given one input (the value of $c$),
we return exactly one output (the value of ${f'}(c)$). The “do
...
(0,0) node (n1) \[text width=60pt,align=center\] [input variable $x$ ]{}
(3,0) node (n2) \[draw,text width=60pt,align=center\] [do something to
$f$ and $x$]{} (6,0) node (n3) \[text width=60pt,align=center\] [return
function ${f'}(x)$]{}; (n1) – (n2); (n2) – (n3);

The output is the “derivative function,” ${f'}(x)$. The ${f'}(x)$
function will take a number $c$ as input and return the derivative of
$f$ at $c$. This calls for a definition.

```{definition, label="thederivative", name="Derivative Function", echo=TRUE}

Let $f$ be a differentiable function on an open interval $I$. The
function $${f'}(x) = \lim_{h\to 0} \frac{f(x+h)-f(x)}{h}$$ is [the
derivative of $f$]{}.\
[Notation:]{}
...
**Important:** The notation ${\displaystyle}\frac{dy}{dx}$ is one
symbol; it is **not** the fraction “$dy/dx$”. The notation, while
somewhat confusing at first, was chosen with care. A fraction–looking
symbol was chosen because the derivative has many fraction–like
properties. Among other places, we see these properties at work when we
talk about the units of the derivative, when we discuss the Chain Rule,
and when we learn about integration (topics that appear in later
sections and chapters).\
Examples will help us understand this definition.\

```{example, label="deriv1", name="Finding the derivative of a function", echo=TRUE}

Let $f(x) = 3x^2+5x-7$ as in Example \@ref(exdervpoint1). Find
${f'}(x)$.

------------------------------------------------------------------------

We apply Definition \@ref(def:thederivative). $$\begin{aligned}
    {f'}(x) &= \lim_{h\to 0} \frac{f(x+h)-f(x)}{h} \\
                    &=  \lim_{h\to 0} \frac{3(x+h)^2+5(x+h)-7-(3x^2+5x-7)}{h}\\
                    &=  \lim_{h\to 0} \frac{3h^2 +6xh+5h}{h}\\
                    &= \lim_{h\to 0} (3h+6x+5)\\
                    &= 6x+5
    \end{aligned}$$ So ${f'}(x) = 6x+5$. Recall earlier we found that
${f'}(1) = 11$ and ${f'}(3) = 23$. Note our new computation of ${f'}(x)$
affirm these facts.
...
```{example, label="deriv2", name="Finding the derivative of a function", echo=TRUE}

Let ${\displaystyle}f(x) = \frac{1}{x+1}$. Find ${f'}(x)$.
...
So ${\displaystyle}{f'}(x) = \frac{-1}{(x+1)^2}$. To practice using our
...
```{example, label="derivsinx", name="Finding the derivative of a function", echo=TRUE}

Find the derivative of $f(x) = \sin x$.

------------------------------------------------------------------------

Before applying Definition \@ref(def:thederivative), note that once this
is found, we can find the actual tangent line to $f(x) = \sin x$ at
$x=0$, whereas we settled for an approximation in Example
\@ref(exdernumapprox). $$\begin{aligned}
        {f'}(x) &= \lim_{h\to 0} \frac{\sin(x+h)-\sin x}{h} & & \left(\text{\scriptsize \parbox{110pt}{\centering Use trig identity $\sin(x+h) = \sin x\cos h+\cos x\sin h$}}\right) \\
                        ...
                        &= \lim_{h\to 0} \left(\frac{\sin x(\cos h-1)}{h} + \frac{\cos x\sin h}{h}\right) & & \left(\parbox{130pt}{\scriptsize use ${\displaystyle}\lim_{h\to 0} \frac{\cos h-1}{h} = 0$ and ${\displaystyle}\lim_{h\to 0} \frac{\sin h}{h} = 1$}\right)\\
                        &=  \sin x\cdot 0 + \cos x \cdot 1 \\
                        &= \cos x\ !
        \end{aligned}$$ We have found that when $f(x) = \sin x$,
${f'}(x) = \cos x$. This should be somewhat surprising; the result of a
...
periodic – it repeats itself on regular intervals. Therefore its rate of
change also repeats itself on the same regular intervals. We should have
known the derivative would be periodic; we now know exactly which
periodic function it is.

Thinking back to Example \@ref(exdernumapprox), we can find the slope
of the tangent line to $f(x)=\sin x$ at $x=0$ using our derivative. We
approximated the slope as $0.9983$; we now know the slope is *exactly*
$\cos 0 =1$.
...
```{example, label="notdiff", name="Finding the derivative of a piecewise defined function", echo=TRUE}
...
ind the derivative of the absolute value function,
$$f(x) = |x| = \left\{\begin{array}{cc} -x & x<0 \\ x & x\geq 0\end{array}.\right.$$
See Figure \@ref(fig:absolutevalue).

We need to evaluate ${\displaystyle}\lim_{h\to0}\frac{f(x+h)-f(x)}{h}.$
...
$${f'}(0) = \lim_{h\to0}\frac{f(0+h)-f(0)}{h}.$$ Since $x=0$ is the
...
following, where we compute the left and right hand limits side by side.

$$\begin{aligned}
\lim_{h\to0^-}\frac{f(0+h)-f(0)}{h} &= \\
\lim_{h\to0^-}\frac{-h-0}{h} &= \\
\lim_{h\to0^-}-1 & =-1\end{aligned}$$
...
At $x=0$, ${f'}(x)$ does not exist; there is a jump discontinuity at 0;
see Figure \@ref(fig:absolutevalueprime). So $f(x) = |x|$ is differentiable
...
```{example, label="diffpiecewise", name="Finding the derivative of a piecewise defined function", echo=TRUE}
...
ind the derivative of $f(x)$, where
${\displaystyle}f(x) = \left\{\begin{array}{cc} \sin x & x\leq \pi/2 \\ 1 & x>\pi/2 \end{array}.\right.$
See Figure \@ref(fig:piecewisesinx1).

Using Example \@ref(exderivsinx), we know that when $x<\pi/2$,
${f'}(x) = \cos x$. It is easy to verify that when $x>\pi/2$,
${f'}(x) = 0$; consider:
$$\lim_{h\to0}\frac{f(x+h) - f(x)}{h} = \lim_{h\to0}\frac{1-1}{h} = \lim_{h\to0}0 =0.$$

So far we have
$${f'}(x) = \left\{\begin{array}{cc} \cos x & x<\pi/2\\ 0 & x>\pi/2\end{array}.\right.$$
We still need to find ${f'}(\pi/2)$. Notice at $x=\pi/2$ that both
pieces of ${f'}$ are 0, meaning we can state that ${f'}(\pi/2)=0$.

Being more rigorous, we can again evaluate the difference quotient limit
at $x=\pi/2$, utilizing again left and right–hand limits:\
...
exists and ${f'}(\pi/2)$ exists (and is 0). Therefore we can fully write
${f'}$ as
$${f'}(x) = \left\{\begin{array}{cc} \cos x & x\leq\pi/2\\ 0 & x>\pi/2\end{array}.\right.$$
...
is shown in Figure \@ref(fig:absolutevalue). Even though the function $f$
in Example \@ref(exdiffpiecewise) is piecewise–defined, the transition
is “smooth” hence it is differentiable. Note how in the graph of $f$ in
Figure \@ref(fig:piecewisesinx1) it is difficult to tell when $f$ switches
from one piece to the other; there is no “corner.”

**Differentiablity on Closed Intervals**\
When we defined the derivative at a point in Definition
\@ref(def:derivativeatapoint), we specified that the interval $I$
...
Recall we also required open intervals in Definition \@ref(def:continuous)
when we defined what it meant for a function to be continuous. Later, we
used one-sided limits to extend continuity to closed intervals. We now
extend differentiability to closed intervals by again considering
one-sided limits.

Our motivation is three-fold. First, we consider “common sense.” In
Example \@ref(exderiv1) we found that when $f(x) =  3x^2+5x-7$,
${f'}(x) = 6x+5$, and this derivative is defined for all real numbers,
...
as well. After all, ${f'}(x)$ is defined at both $x=0$ and $x=1$.
...
\@ref(sec:arclength) where the derivative plays a role in measuring the
length of a curve.

After a formal definition of differentiability on a closed interval, we
explore the concept in an example.

```{definition, label="diffclosed", name="Differentiability on a Closed Interval", echo=TRUE}
...
$\lim_{x\to a^+}{f'}(x)$ and $\lim_{x\to b^-}{f'}(x)$. This is often
easier to evaluate than the limit of the difference quotient.\

```{example, label="diffclosed1", name="Differentiability at an endpoint", echo=TRUE}

Consider $f(x) = \sqrt{x}=x^{1/2}$ and $g(x) = \sqrt{x^3}=x^{3/2}$. The
domain of each function is $[0,\infty)$. It can be shown that each is
differentiable on $(0,\infty)$; determine the differentiability of each
at $x=0$.
...
We state (without proof) that ${f'}(x) = 1/\big(2\sqrt{x}\big)$. Note
that $\lim_{x\to0^+}{f'}(x) = \infty$; this limit was easier to evaluate
than the limit of the difference quotient, though it required us to
already know the derivative of $f$.
...
We state (without proof) that ${g'}(x) = 3\sqrt{x}/2$. Note that
$\lim_{x\to0^+}{g'}(x) = 0$; again, this limit is easier to evaluate
than the limit of the difference quotient.

The two functions are graphed in Figure \@ref(fig:diffclosed1). Note how
$f(x) = \sqrt{x}$ seems to “go vertical” as $x$ approaches 0, implying
the slopes of its tangent lines are growing toward infinity. Also note
how the slopes of the tangent lines to $g(x)= \sqrt{x^3}$ approach 0 as
$x$ approaches 0.
...
\@ref(chapter:appofint), where we measure the lengths of curves.\
