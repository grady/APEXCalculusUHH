...
height. In Figure \@ref(fig:rie1b), the rectangle drawn on the interval
$[2,3]$ has height determined by the Left Hand Rule; it has a height of
...
In the figure, the rectangle drawn on $[0,1]$ is drawn using $f(1)$ as
its height; this rectangle is labeled "RHR.".

The **Midpoint Rule** says that on each subinterval, evaluate the
function at the midpoint and make the rectangle that height. The
rectangle drawn on $[1,2]$ was made using the Midpoint Rule, with a
height of $f(1.5)$. That rectangle is labeled "MPR."

These are the three most common rules for determining the heights of
approximating rectangles, but one is not forced to use one of these
three methods. The rectangle on $[3,4]$ has a height of approximately
$f(3.53)$, very close to the Midpoint Rule. It was chosen so that the
area of the rectangle is *exactly* the area of the region under $f$ on
$[3,4]$. (Later you'll be able to figure how to do this, too.)

The following example will approximate the value of
$\int_0^4 (4x-x^2)\ dx$ using these rules.\

```{example, label="rie2", echo=TRUE}
...
::: {#fig:rie2 .tab}
**Table 1**. caption: Approximating $\int_0^4(4x-x^2)\ dx$ in Example
\@ref(exm:rie2). In (a), the Left Hand Rule is used; in (b), the Right
Hand Rule is used; in (c), the Midpoint Rule is used.
:::
...
Figure \@ref(fig:rie2)(b) shows 4 rectangles drawn under $f$ using the
Right Hand Rule; note how the $[3,4]$ subinterval has a rectangle of
height 0.
...
Figure \@ref(fig:rie2)(c) shows 4 rectangles drawn under $f$ using the
Midpoint Rule.
...
\
**Summation Notation** It is hard to tell at this moment which is a
...
The upper case sigma represents the term "sum." The index of summation
in this example is $i$; any symbol can be used. By convention, the index
takes on only the integer values between (and including) the lower and
upper bounds.
...
caption: >
    Dividing $[0,4]$ into 16 equally spaced subintervals.
...
**Midpoint Rule:**
$\displaystyle\sum_{i=1}^{16} f\left(\frac{x_i+x_{i+1}}2\right)\Delta x$
...
Notice Equation \@ref(eq:rie7); by changing the 16's to 1,000's (and
appropriately changing the value of $\Delta x$), we can use that
equation to sum up 1000 rectangles!

We do so here, skipping from the original summand to the equivalent of
Equation \@ref(eq:rie7) to save space. Note that
...
caption: >
    ...
    and 16 evenly spaced subintervals.
...
```{geogebra, label="riedef"}

caption: >
    An example of a general Riemann sum to approximate $\int_0^4(4x-x^2)\ dx$.
...
Consider
$\displaystyle\int_a^b f(x) \ dx \approx \sum_{i=1}^n f(c_i)\Delta x_i.$
...
Using Key Idea \@ref(idea:riemann), we know
...
each step for full understanding. $$\begin{aligned}
        \int_0^4(4x-x^2)\ dx &\approx \sum_{i=1}^n f(x_{i+1})\Delta x \\
                                            ...
                                            &= \left(\frac{16\Delta x}{n}\right)\cdot \frac{n(n+1)}{2} - \left(\frac{16\Delta x}{n^2}\right)\frac{n(n+1)(2n+1)}{6} \quad \left(\scriptsize \centering recall $\Delta x = 4/n$\right)\\
...
(these rectangles are shown in Figure \@ref(fig:rie9)). Using $n=100$
